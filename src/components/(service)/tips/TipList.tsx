...
export default function TipList() {
	const { data, hasTipList, hasNextPage, fetchNextPage, isLoading } = useTipList();
	...
	return (
		<>
			<ul className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
				{!hasTipList && <Wip icon={<Sparkle size={18} />} message={`Empty Data`} />}
				{data?.map(({ id, title, imgSrc, url, category, created_at }) => (
					<li key={id} className="flex items-center bg-light border border-muted rounded-lg">
						<Link href={`/tips/${id}`} className="flex flex-col justify-between gap-4 p-3 w-full h-full rounded-lg">
							{imgSrc ? (
								<Image
									src={imgSrc}
									alt={title}
