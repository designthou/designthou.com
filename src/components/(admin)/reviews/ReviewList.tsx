...
import { AnimateLoader, Button, Table, TableBody, TableCell, TableHead, TableHeader, TableRow } from '@/components';
...
export default function AdminReviewList({ category }: ReviewListProps) {
	const { reviews, hasNextPage, fetchNextPage, isLoading, noticeReview, totalCount } = useReviewList({ category });
	...
	return (
		<>
			<span className="absolute top-4 right-4 font-black text-2xl text-foreground">{totalCount}</span>
			...
			<Table className="border border-muted rounded-lg">
				...
				<TableBody className="rounded-lg">
					<TableRow className="bg-orange-50 font-semibold rounded-lg">
						...
						<TableCell>{convertSupabaseDateToShortHumanReadable(noticeReview.created_at)}</TableCell>
						<TableCell className="text-right">{noticeReview.view_count}</TableCell>
					</TableRow>
					{reviews.map(review => (
						<TableRow key={review.id} className="rounded-lg">
							...
							<TableCell>{convertSupabaseDateToShortHumanReadable(review.created_at)}</TableCell>
