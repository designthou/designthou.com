...
import { revalidateNewsAction } from '@/app/actions';
...
export default function NewsForm({
	...
	const { mutate: add, isPending: isAddPending } = useAddNewsMutation({ closeForm, revalidateNewsAction });
	const { mutate: edit, isPending: isEditPending } = useEditNewsMutation({ closeForm, revalidateNewsAction });
		...
		<Form {...form}>
			<form onSubmit={form.handleSubmit(onSubmit)} className={cn('grid items-start gap-6', className)}>
				...
				<Button type="submit" size="lg">
					{isAddPending || isEditPending ? <AnimateLoader /> : type === 'add' ? 'Submit' : 'Edit'}
				</Button>
			</form>
		</Form>
	);
