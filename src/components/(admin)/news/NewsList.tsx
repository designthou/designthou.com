...
import { AnimateLoader, DeleteNewsAlertDialog, Wip, EditNewsContext } from '@/components';
...
export default function NewsList({ year }: NewsListProps) {
	const { newsList, hasNewsList, hasNextPage, fetchNextPage, isLoading } = useNewsList({ year });
	...
	return (
		<>
			<ul className="grid grid-cols-1 gap-3 md:grid-cols-2">
				{!hasNewsList && <Wip icon={<Sparkle size={18} />} message={`Empty Data on ${year} year`} />}
				{newsList?.map(({ id, title, url, category, created_at }, idx) => (
					<li key={`${id}-${title}-${idx}`} className="flex items-center bg-light border border-muted rounded-lg">
						<div className="flex flex-col gap-4 p-3 w-full h-full">
							<div className="flex flex-col justify-between gap-4 w-full h-full">
								...
								<div className="ui-flex-center-between">
									<span className="text-gray-700 text-xs sm:text-sm">{convertSupabaseDateToShortHumanReadable(created_at)}</span>
