	...
	return (
		<section className="flex flex-col gap-4 my-4 p-4 mx-auto max-w-200 md:my-8">
			<Button asChild variant="secondary" className="w-fit">
				<Link href={route.SERVICE.OPEN_SOURCE_AI}>
					<ArrowLeft /> Go back
				</Link>
			</Button>
			...
			<div className="grid grid-cols-1 gap-8 md:grid-cols-5">
				<div className="md:col-span-2 ui-flex-center w-fit border border-muted rounded-lg">
					{previewFileName ? (
						<Image
							src={supabaseServerClient.storage.from(STORAGE_BUCKETS.OPEN_SOURCE_AI_PREVIEW).getPublicUrl(previewFileName).data.publicUrl}
							alt={previewFileName}
							width={300}
							height={300}
				...
				<div className="md:col-span-3 flex flex-col justify-between gap-4 w-full">
					<div className="flex flex-col gap-4 p-4 bg-light rounded-lg">
					...
					<div className="flex flex-col gap-4 p-4 bg-light rounded-lg">
						<DownloadButton fileName={previewFileName.replace('.png', '.ai')} fileType={'ai'} />
						{!user?.user && (
