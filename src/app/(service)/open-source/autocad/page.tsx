...
export default async function AutocadOpenSourceListPage() {
	const supabase = createStaticClient();

	const [{ data: files, error }, { data: previewFiles }] = await Promise.all([
		supabase.storage.from(STORAGE_BUCKETS.OPEN_SOURCE_CAD).list(''),
		supabase.storage.from(STORAGE_BUCKETS.OPEN_SOURCE_CAD_PREVIEW).list(''),
	]);
	...
	return (
		<section className="flex flex-col gap-4 my-4 p-4 mx-auto max-w-300 md:my-8">
			...
			<h2 className="page-title">Open Source (.dwg)</h2>

			<div className="grid grid-cols-2 gap-4 mt-8 md:grid-cols-4">
				{fileList?.map(({ name, baseName, previewFileName }, idx) => (
					<Link
						key={name}
						href={`/open-source/autocad/${previewFileName}`}
						className="relative ui-flex-center border border-gray-100 bg-white rounded-lg hover:bg-light text-gray-600 hover:ring-2 ring-gray-200 transition-all">
						{previewFileName ? (
							<Image
								src={supabase.storage.from(STORAGE_BUCKETS.OPEN_SOURCE_CAD_PREVIEW).getPublicUrl(previewFileName).data.publicUrl}
								alt={baseName}
								width={300}
								height={300}
								placeholder="blur"
