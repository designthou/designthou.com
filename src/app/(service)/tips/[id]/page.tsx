...
export default async function TipVideoPage({ params }: { params: Promise<{ id: string }> }) {
	...
	return (
		<section className="mx-auto p-4">
			...
			<div className="flex flex-col justify-between gap-2 mt-8 sm:items-center sm:flex-row">
			...
			<div className="mt-8 p-4 w-full bg-light rounded-lg">
				...
				<dl className="flex flex-col gap-8 px-3 py-1.5 w-full">
					<dt className="flex items-center gap-4">
						<span className="font-semibold">디자인도우 건축 디자인 오픈채팅방</span>
						<Button type="button" variant="default" asChild>
							<Link href="https://open.kakao.com/o/gzL5v2Hd" target="_blank" rel="noopenner noreferrer">
								바로가기
								<ArrowUpRight size={18} />
							</Link>
						</Button>
					</dt>
					<dd className="flex flex-col gap-2 w-full sm:flex-row sm:items-center">
						<p>건축, 인테리어 작업에 도움이 필요할 때 언제든지 질문 가능합니다</p>
						<div className="flex items-center gap-2 px-3 py-1.5 bg-light border border-orange-200 rounded-lg">
							<span>참여코드</span>-
							<span className="inline-block px-3 py-1.5 font-black text-white bg-gradient-orange-100 rounded-md">디도 (eleh)</span>
						</div>
					</dd>
				</dl>
			</div>
